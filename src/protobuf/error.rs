...
pub struct ErrorKindUnexpectedDecoder {
    // フィールドを追加できるようにするため 1 フィールドしかないが `Fields` を使う
    inner: MessageDecoder<Fields<(Optional<FieldDecoder<F1, ObjectVersionDecoder>>,)>>,
}
impl_message_decode!(
    ErrorKindUnexpectedDecoder,
    (Option<ObjectVersion>,),
    |t: (Option<u64>,)| Ok((t.0.map(ObjectVersion),))
);
...
pub struct ErrorKindUnexpectedEncoder {
    // フィールドを追加できるようにするため 1 フィールドしかないが `Fields` を使う
    inner: MessageEncoder<Fields<(Optional<FieldEncoder<F1, ObjectVersionEncoder>>,)>>,
}
impl_sized_message_encode!(
    ErrorKindUnexpectedEncoder,
    (Option<ObjectVersion>,),
    |item: Self::Item| (item.0.map(|v| v.0),)
);
